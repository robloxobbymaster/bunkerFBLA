...
func show_completion(speed: int = SPEED) -> void:
	actualSpeed = speed
	AudioManager.play(AudioManager.SoundIds.SUCCESS_SFX)
	target_pos =  show_pos
	
func hide_completion(speed: int = SPEED) -> void:
	actualSpeed = speed
	target_pos = hidden_pos

func _process(delta: float) -> void:
	%CompletedLabel.global_position = lerp(%CompletedLabel.global_position,target_pos, delta*actualSpeed)
